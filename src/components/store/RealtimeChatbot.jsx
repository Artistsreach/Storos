...
const RealtimeChatbot = () => {
  ...
  const { currentStore, addToCart: contextAddToCart, getProductById, updateQuantity: contextUpdateQuantity } = useStore();
  ...
  const clientFindAndOpenProduct = (args) => {
    ...
    if (foundProduct) {
      ...
      if (foundProduct.image?.src?.small && typeof foundProduct.image.src.small === 'string') {
      ...
      } else if (foundProduct.image?.src?.large && typeof foundProduct.image.src.large === 'string') {
      ...
      } else if (foundProduct.image && typeof foundProduct.image === 'string') {
        primaryImageUrl = foundProduct.image;
      } else if (foundProduct.images && Array.isArray(foundProduct.images) && foundProduct.images.length > 0 && foundProduct.images[0]?.src) {
        primaryImageUrl = foundProduct.images[0].src;
      } else if (foundProduct.featuredImage?.url) {
