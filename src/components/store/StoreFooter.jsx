  ...
  const handleSaveText = async (field, value, index = null) => {
    if (storeId) {
      try {
        ...
        if (field === 'storeNameFooter') { // Use a distinct key if name is also in header
          await updateStore(storeId, { name: value }); // Assuming footer name is the main store name
        ...
        // Only call updateStore if newContent has changed from content
          ...
          <div className="space-y-3">
            ...
            <div className="flex space-x-3 pt-2">
              {socialLinks.map(link => {
                const IconComponent = IconMap[link.iconName] || Facebook; // Default to Facebook if iconName is missing/invalid
                return (
                  <a
                    key={link.label}
                    href={link.href}
