...
import { fetchPexelsImages, fetchPexelsVideos } from "./utils.jsx";
...
const modelConfig = {
  ...
  systemInstruction: `
    ...
    The website should be visually appealing, responsive, and fully functional.
...
export function startChat() {
  if (!API_KEY) {
    console.error("[geminiTemplateGeneration] VITE_GEMINI_API_KEY is not available.");
...
export async function* continueChat(chat, prompt) {
  ...
  while (attempts < maxAttempts) {
    attempts++;
    try {
      console.log(`[geminiTemplateGeneration] Attempt ${attempts} to continue chat...`);
      ...
      for await (const chunk of stream) {
        ...
        if (chunk.promptFeedback && chunk.promptFeedback.blockReason) {
            console.error(`[geminiTemplateGeneration] Prompt was blocked during streaming. Reason: ${chunk.promptFeedback.blockReason}`);
      ...
      if (htmlContent && htmlContent.trim().toLowerCase().startsWith('<!doctype html>')) {
        ...
        console.log(`[geminiTemplateGeneration] Successfully generated website on attempt ${attempts}.`);
        yield { type: 'result', content: { title, primaryColor, htmlContent } };
        return;
      } else {
        console.warn(`[geminiTemplateGeneration] Attempt ${attempts} did not return valid HTML. Retrying...`);
      }
    } catch (error) {
      console.error(`[geminiTemplateGeneration] Error during attempt ${attempts}:`, error);
