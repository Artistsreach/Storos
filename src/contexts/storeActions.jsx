...
export const generateStoreFromWizardData = async (wizardData, { fetchPexelsImages = utilFetchPexelsImages, generateId = utilGenerateId } = {}) => {
    ...
    // Destructure with defaults to prevent errors if wizardData or its properties are undefined/null
    const { 
      productType = 'general', 
      storeName = 'My Store', 
      logoUrl = '', 
      products: wizardProductsInfo, // Renamed from wizardProducts to wizardProductsInfo for clarity
      collections: wizardCollectionsInfo, // Added for clarity
      prompt = '', 
      content: wizardContent 
    } = wizardData || {};

    let finalProducts = [];
    // Check if wizardProductsInfo and its items array exist
    if (wizardProductsInfo && Array.isArray(wizardProductsInfo.items)) {
        if (wizardProductsInfo.source === 'ai') {
            finalProducts = wizardProductsInfo.items.map(item => ({
                id: item.id || `temp-product-${generateId()}`, // Use existing temp ID, or generate if somehow missing
                name: item.name || 'AI Product',
                price: parseFloat(item.price) || 0,
                description: item.description || 'AI generated description.',
                image: { 
                    id: generateId(), // Image ID can be new
                    src: { medium: item.imageUrl || `https://via.placeholder.com/400x400.png?text=AI+Product` },
                    alt: item.name || 'AI Product',
                },
                rating: (Math.random() * 1.5 + 3.5).toFixed(1),
                stock: Math.floor(Math.random() * 80) + 20,
            }));
        } else if (wizardProductsInfo.source === 'manual') {
            finalProducts = wizardProductsInfo.items.map(p => ({
                id: p.id || `temp-product-${generateId()}`, // Use existing temp ID
                name: p.name || 'Manual Product',
                price: parseFloat(p.price) || 0,
                description: p.description || generateAIProductDescriptions(productType, p.name || 'Manual Product'),
                image: {
                    id: generateId(),
                    src: { medium: p.imageUrl || `https://via.placeholder.com/400x400.png?text=${encodeURIComponent(p.name || 'Manual Product')}` },
                    alt: p.name || 'Manual Product',
                },
                rating: (Math.random() * 1.5 + 3.5).toFixed(1),
      ...
      console.warn("[generateStoreFromWizardData] wizardData.products or wizardData.products.items is undefined. No products will be generated from wizard data.");
    }
  ...
  // Default nav link labels
  const defaultNavLinkLabels = ['Home', 'Products', 'Features', 'Contact'];
  const finalContent = {
    ...aiContent,
    navLinkLabels: wizardData.content?.navLinkLabels || defaultNavLinkLabels, // Use from wizard if provided, else default
    // Ensure other content fields from wizardData.prompt (which populates aiContent) are preserved or merged
    heroTitle: wizardData.content?.heroTitle || aiContent.heroTitle,
    heroDescription: wizardData.content?.heroDescription || aiContent.heroDescription,
    // Assuming other specific content fields like feature titles/descriptions are handled by aiContent or wizardData if structured
  };
    ...
    return {
      ...
      collections: (wizardCollectionsInfo && Array.isArray(wizardCollectionsInfo.items)) 
        ? wizardCollectionsInfo.items.map(collection => ({
            id: collection.id || `temp-collection-${generateId()}`, // Use existing temp ID or generate
            name: collection.name || 'Unnamed Collection',
            description: collection.description || '',
            imageUrl: collection.imageUrl || '',
            product_ids: Array.isArray(collection.product_ids) ? collection.product_ids : [],
          }))
        : [], // Default to empty array if collections info is missing
      ...
      content: finalContent, // Use the merged content object
      reviews: aiContent.reviews || [], // Ensure reviews are at the top level
      data_source: 'wizard',
...
export const generateStoreFromPromptData = async (
  ...
  const numCollectionsToGenerate = Math.min(Math.max(1, Math.floor(generatedProducts.length / 2)), 3); // Aim for 1-3 collections
  const existingCollectionNamesForAI = [];
    ...
    for (let i = 0; i < numCollectionsToGenerate; i++) {
      try {
        const collectionData = await generateCollectionWithGemini(
          storeType,
          brandName,
          generatedProducts, // Pass all generated products for context
          existingCollectionNamesForAI
        );

        if (collectionData && !collectionData.error && collectionData.name) {
          existingCollectionNamesForAI.push(collectionData.name);
          
          // Assign products to this collection (simple distribution for now)
          // Example: assign a slice of products to each collection
          const productsPerCollection = Math.ceil(generatedProducts.length / numCollectionsToGenerate);
          const startIndex = i * productsPerCollection;
          const endIndex = Math.min(startIndex + productsPerCollection, generatedProducts.length);
          const productIdsForThisCollection = generatedProducts.slice(startIndex, endIndex).map(p => p.id);

          generatedCollections.push({
            id: `temp-collection-${generateId()}`,
            name: collectionData.name,
            description: collectionData.description,
            imageUrl: collectionData.imageUrl, // This comes from Pexels search in generateCollectionWithGemini
            product_ids: productIdsForThisCollection,
          });
          console.log(`[generateStoreFromPromptData] Collection "${collectionData.name}" generated with ${productIdsForThisCollection.length} products.`);
        } else {
          console.warn(`[generateStoreFromPromptData] Failed to generate collection data for iteration ${i}:`, collectionData?.error);
        }
      } catch (error) {
        console.error(`[generateStoreFromPromptData] Error generating collection ${i}:`, error);
      }
    }
  }
  if (generatedCollections.length === 0 && numCollectionsToGenerate > 0) {
    console.warn(`[generateStoreFromPromptData] No collections were generated successfully.`);
  }


  // Default nav link labels for prompt-generated stores
  const defaultNavLinkLabelsPrompt = ['Home', 'Products', 'Features', 'Contact'];
  const finalContentPrompt = {
    ...aiContent,
    navLinkLabels: defaultNavLinkLabelsPrompt,
  };
  ...
  return {
    ...
    collections: generatedCollections, // Add generated collections
    hero_image: heroImages[0] || { src: { large: 'https://via.placeholder.com/1200x800.png?text=Hero+Image' }, alt: 'Placeholder Hero Image' },
    hero_video_url: heroVideos[0]?.url || null,
    hero_video_poster_url: heroVideos[0]?.image || null,
    logo_url: actualLogoUrl, // Use the generated or placeholder logo URL
      ...
      content: finalContentPrompt, // Use the merged content object
      reviews: aiContent.reviews || [], // Ensure reviews are at the top level
      data_source: 'ai',
